...
class Car(pmo.block):
    ...
    def evp_shift_1_tolist(self,EVGENP):
        for park,ev,arr_idx,end_idx,bus in self.PARK:
            for t in self.T:
                self.ShiftAbs1[park,t]=pmo.constraint(M*self.FLEX[park,ev,arr_idx,end_idx,bus,t]>=1e3*(EVGENP[park,ev,arr_idx,end_idx,bus,t]-self.EVPPLAN[park,ev,arr_idx,end_idx,bus,t]))
    ...
    def evp_shift_2_tolist(self,EVGENP):
        for park,ev,arr_idx,end_idx,bus in self.PARK:
            for t in self.T:
                self.ShiftAbs2[park,t]=pmo.constraint(M*self.FLEX[park,ev,arr_idx,end_idx,bus,t]>=-1e3*(EVGENP[park,ev,arr_idx,end_idx,bus,t]-self.EVPPLAN[park,ev,arr_idx,end_idx,bus,t]))    
