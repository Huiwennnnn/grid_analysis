...
path = f"/cluster/home/huiluo/mt/grid_369_0/DistFlow/{scenario_year}_{month:02d}_{day:02d}_{monitor_hr}_v2g"
...
# Create a figure for vm_pu_results
...
# Create a figure for trafo_loading_results
plt.figure(figsize=(10*monitor_hr//24, 6))
...
# Create a figure for line_loading_results
plt.figure(figsize=(10*monitor_hr//24, 6))
...
class Car(pmo.block):
    def __init__(self,cars,EVGENP,monitor_hr,PARK=None,EVBAT=None,T=None,ev_load_profile=None): 
        ...
        # self.BUFFER = pmo.variable(domain=pmo.NonNegativeReals,value=0)

        self.FLEX = pmo.variable_dict()
        for person in self.EVBAT:
            self.FLEX[person]=pmo.variable(domain=pmo.Binary,value=0)

        # self.NEXT_E_slack = pmo.variable_dict()
        # for park,ev,arr_idx,end_idx,bus in self.PARK:
        #     self.NEXT_E_slack[park,ev,arr_idx,end_idx,bus]=pmo.variable(domain=pmo.NonNegativeReals,value=0)
        ...
        self.PersonShiftAbs = pmo.expression_dict()
        self.person_shift_abs_tolist()

        self.DetermineShift1=pmo.constraint_dict()
        for person in self.EVBAT:
            self.DetermineShift1[person] = pmo.constraint(self.PersonShiftAbs[person]<=M*self.FLEX[person])

        self.DetermineShift2=pmo.constraint_dict()
        for person in self.EVBAT:
            self.DetermineShift2[person] = pmo.constraint(self.PersonShiftAbs[person]>=1e-3-M*(1-self.FLEX[person]))

        self.Participate =pmo.constraint(sum(self.FLEX[person] for person in self.EVBAT) <= 0.3*cars)#+self.BUFFER)
        ...
        for park,ev,arr_idx,end_idx,bus in self.PARK:
            for t in self.T:
                if t==end_idx and t<monitor_hr-1 and arr_idx!=end_idx:
                    self.NextE[park]=pmo.constraint(self.SOE[ev,t]-EVGENP[park,ev,arr_idx,end_idx,bus,t]*self._PARK_HR[park,ev,arr_idx,end_idx,bus,t]>=self._NEXT_E[park,ev,arr_idx,end_idx,bus]) #+self.NEXT_E_slack[park,ev,arr_idx,end_idx,bus]
    ...
    def person_shift_abs_tolist(self):
        for person in self.EVBAT:
            self.PersonShiftAbs[person] = pmo.expression(sum(self.EVP_SHIFT_ABS[park,ev,arr_idx,end_idx,bus,t] for park,ev,arr_idx,end_idx,bus in m.PARK for t in m.T if person==ev))     
...
m.Cars = Car(cars,m.Grids.EVGENP,monitor_hr,m.PARK,m.EVBAT,m.T,ev_load_profile) # EV part



# PV and Dayend soe slack
# m.obj = pmo.objective(
#     -1e-3*sum(m.Grids.PVGENP[pv,bus,t] for pv, bus in m.PV for t in m.T)
#     +1e3*sum(m.Cars.SOE_dayend_slack[person] for person in m.EVBAT)
#     +sum(m.Cars.EVP_SHIFT_ABS[park,ev,arr_idx,end_idx,bus,t] for park,ev,arr_idx,end_idx,bus in m.PARK for t in m.T)
#     +1e3*sum(m.Grids.V_slack[bus,t] for bus in m.BUS for t in m.T),
#     sense=pmo.minimize
#     )

# # only pv
# m.obj = pmo.objective(
#     -sum(m.Grids.PVGENP[pv,bus,t] for pv, bus in m.PV for t in m.T)
#     +sum(m.Cars.EVP_SHIFT_ABS[park,ev,arr_idx,end_idx,bus,t] for park,ev,arr_idx,end_idx,bus in m.PARK for t in m.T)
#     +1e3*sum(m.Grids.V_slack[bus,t] for bus in m.BUS for t in m.T),
#     sense=pmo.minimize
#     )

# only dayend soe slack
m.obj = pmo.objective(
    +1e6*sum(m.Cars.SOE_dayend_slack[person] for person in m.EVBAT)
    +sum(m.Cars.EVP_SHIFT_ABS[park,ev,arr_idx,end_idx,bus,t] for park,ev,arr_idx,end_idx,bus in m.PARK for t in m.T)
...
dayendsoe_df.to_csv(f"{save_path}/dayendsoe_df.csv")
